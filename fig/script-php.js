...
async function initLetters() {
  ...
  await Promise.all(letters.map(async (letter) => {
    ...
    // 2. IMMEDIATELY show the first frame so nothing is "missing"
    if (letter.frames.length > 0) {
      letter.el.src = letter.frames[0].src;
    }
  }));

  // 3. Now that all letters are visible, start the shifting animation
  startSequentialAnimation();
}

function startSequentialAnimation() {
  let currentLetterIdx = 0;
  const fps = 12; // Adjusted for a smoother "shift"
  ...
  function animate(time) {
    if (time - lastTime >= interval) {
      ...
      if (letter.frames && letter.frames.length > 0) {
        // Move this specific letter to its next frame
        letter.index = (letter.index + 1) % letter.frames.length;
        letter.el.src = letter.frames[letter.index].src;
      }

      // 4. Shift focus to the next letter for the next tick
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  let currentSize = 360;
  let targetSize = 360;
  const ease = 0.15; // Speed of the scale transition (0.01 to 1.0)
  ...
  function animate() {
    if (liveCursor) {
      // Smoothly interpolate currentSize toward targetSize
  ...
  function init() {
    ...
    document.addEventListener("pointermove", (e) => {
      ...
      // Check if hovering over a link to set the target scale
      const isOverLink = e.target.closest('a');
      targetSize = isOverLink ? 100 : 360;

      if (needsNewStamp) {
        liveCursor = createStamp(x, y, currentSize);
        needsNewStamp = false;
      } else {
        liveCursor.x = x;
        liveCursor.y = y;
    ...
    document.addEventListener("pointerdown", (e) => {
      ...
      if (pointer.isTouch()) {
        ...
        if (liveCursor) {
          liveCursor.x = x;
          liveCursor.y = y;
          // Freeze the current state into the stamps array
